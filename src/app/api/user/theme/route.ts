...
export async function PUT(request: Request) {
  ...
  const prefs: ThemePreferences = {
    ...
    fontSize: ['small', 'medium', 'large', 'xlarge'].includes(body.fontSize)
      ? body.fontSize
      : defaultPrefs.fontSize,
    layoutDensity: ['compact', 'comfortable', 'spacious'].includes(body.layoutDensity)
      ? body.layoutDensity
      : defaultPrefs.layoutDensity,
    reducedMotion: typeof body.reducedMotion === 'boolean'
      ? body.reducedMotion
      : defaultPrefs.reducedMotion,
    accentColor: ['blue', 'purple', 'teal', 'rose', 'amber', 'emerald'].includes(body.accentColor)
      ? body.accentColor
